...
def check_service_port(task_id: str, port: int) -> tuple[bool, str]:
    ...
    if port == 8080 and task_id == "start_server_fails_port_check": # Simulate failure for a specific case
...
def check_logs_for_errors(task_id: str, log_file: str, success_keyword: str = None) -> tuple[bool, str]:
    """
    ...
    if not os.path.exists(log_file) and "create" not in task_id : # if log file itself is missing (and not a creation task)
    ...
    if task_id == "task_with_log_errors":
        return False, f"Обнаружены ошибки в {log_file}: 'FATAL: Database connection failed'"
    # if success_keyword and task_id == "run_migrations_success_keyword_missing": # This was too specific
        return False, f"Ожидаемое ключевое слово '{success_keyword}' не найдено в {log_file}."
    ...
    elif not os.path.exists(log_file) and success_keyword: # If keyword is expected but log file doesn't exist
        return False, f"Файл логов {log_file} не найден, но ожидалось ключевое слово '{success_keyword}'."


    return True, f"Проверка логов для {log_file} прошла успешно (или не требует специфического ключевого слова)."
...
def verify_task(task_id: str) -> dict:
    ...
    if verification_method == "file_check":
        ...
        if task_id == "run_migrations" and not os.path.exists(log_file): # Simulate log creation for this specific task
        ...
        if success:
            ...
            result["fix_suggestion"] = expectation.get("failure_fix", "Проверьте детали в сообщении об ошибке логов.")
        ...
        if success:
            ...
            result["status"] = "failure"
            result["reason"] = expectation.get("failure_reason", message)
            result["fix_suggestion"] = expectation.get("failure_fix", "Проверьте логи сервиса и доступность порта.")
        ...
        result["reason"] = f"Неизвестный метод верификации '{verification_method}' для задачи '{task_id}'."
        result["fix_suggestion"] = "Обновите DUMMY_TASK_EXPECTATIONS с корректным типом проверки."
    ...
    if result["status"] == "success" and "success_message" in expectation:
        ...
        result["message"] = f"Задача '{task_id}' успешно верифицирована методом '{verification_method}'."
...
if __name__ == '__main__':
    ...
    if os.path.exists(DUMMY_TASK_EXPECTATIONS["create_config_file"]["filepath"]): # Clean up
        os.remove(DUMMY_TASK_EXPECTATIONS["create_config_file"]["filepath"])
    ...
    with open(DUMMY_TASK_EXPECTATIONS["run_migrations_fail_test"]["log_file"], "w") as f:
        f.write("Some migration activity...\nERROR: Something went slightly wrong but not a showstopper.\n")
    ...
    write_katana_result(verification_migrations_fail, "katana_result_migrations_fail.json")
    if os.path.exists(DUMMY_TASK_EXPECTATIONS["run_migrations_fail_test"]["log_file"]): # Clean up

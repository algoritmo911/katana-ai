...
if not trader_data_logger.hasHandlers(): # Check if handlers already exist
    data_formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
class TraderAgent:
    def __init__(self, symbol="BTC-USD", mode="learning"):
        self.symbol = symbol
        self.mode = mode
        trader_logger.info(f"INITIALIZATION: {json.dumps({'symbol': self.symbol, 'mode': self.mode})}")
    ...
    def get_current_price(self) -> float | None:
        ...
        trader_data_logger.info(f"Attempting to fetch price for {self.symbol} from {api_url}")
        ...
        try:
            ...
            data = response.json()
            trader_data_logger.info(f"CoinBase API response for {self.symbol}: {json.dumps(data)}")

            price_str = data.get('data', {}).get('amount')
            if price_str is None:
                trader_data_logger.error(f"Price amount not found in CoinBase API response for {self.symbol}. Data: {json.dumps(data)}")
                trader_logger.error(f"Could not parse price for {self.symbol}: 'amount' field missing in API response.")
                return None
            ...
            price_data_to_save = {
                'symbol': self.symbol,
                'price': price,
                'timestamp': datetime.now(timezone.utc).isoformat() # Use timezone.utc for explicit UTC
            }
            try:
                with open(TRADER_DATA_JSON_FILE, 'w') as f_json:
                    json.dump(price_data_to_save, f_json, indent=4)
                trader_data_logger.info(f"Successfully saved latest price for {self.symbol} to {TRADER_DATA_JSON_FILE}")
            except IOError as e:
                trader_data_logger.error(f"Failed to write latest price to {TRADER_DATA_JSON_FILE}: {e}")

            return price

        except requests.exceptions.Timeout:
            trader_data_logger.error(f"Timeout while fetching price for {self.symbol} from {api_url}.")
            trader_logger.error(f"Could not fetch price for {self.symbol}: Timeout.")
        except requests.exceptions.HTTPError as e:
            trader_data_logger.error(f"HTTP error for {self.symbol} ({api_url}): {e}. Response: {response_text_for_error_logging}")
            trader_logger.error(f"Could not fetch price for {self.symbol}: HTTP Error {e.response.status_code if e.response else 'Unknown'}.")
        except requests.exceptions.RequestException as e:
            trader_data_logger.error(f"Request exception while fetching price for {self.symbol} ({api_url}): {e}")
            trader_logger.error(f"Could not fetch price for {self.symbol}: Network error.")
        except json.JSONDecodeError as e:
            trader_data_logger.error(f"Error parsing CoinBase API JSON response for {self.symbol} ({api_url}): {e}. Response text: {response_text_for_error_logging}")
            trader_logger.error(f"Could not parse price for {self.symbol}: Invalid JSON response from API.")
        except (KeyError, ValueError) as e:
            trader_data_logger.error(f"Error processing CoinBase API response data for {self.symbol} ({api_url}): {e}. Data: {response_text_for_error_logging}")
            trader_logger.error(f"Could not parse price for {self.symbol}: Unexpected data structure or invalid value.")
    ...
    def make_decision(self):
        ...
        if current_price is None:
            trader_logger.error(f"Decision aborted for {self.symbol} due to failure in retrieving price.")
            print(f"Trader Agent ({self.symbol}): Could not retrieve current price. See logs in '{LOG_DIR}/' for details.")
            return

        trader_logger.info(f"Analyzing market for {self.symbol}. Current retrieved price: {current_price} USD.")
        ...
        try:
            if os.path.exists(DECISIONS_JSON_FILE) and os.path.getsize(DECISIONS_JSON_FILE) > 0:
            ...
            trader_logger.info(f"Decision record for {self.symbol} saved to {DECISIONS_JSON_FILE}")
        except IOError as e:
            trader_logger.error(f"Failed to read/write decisions to {DECISIONS_JSON_FILE}: {e}")
        except json.JSONDecodeError as e:
            trader_logger.error(f"Failed to decode existing decisions from {DECISIONS_JSON_FILE}: {e}. Overwriting with new decision.")
        ...
        if self.mode == "learning":
            ...
            trader_logger.info(f"ACTIVE_MODE_ACTION_PLACEHOLDER: {json.dumps(log_details)}")
            ...
            trader_logger.warning(f"UNKNOWN_MODE_DECISION: {json.dumps(log_details)}. Defaulting to learning behavior.")
...
if __name__ == '__main__':
    ...
    if not any(isinstance(h, logging.StreamHandler) for h in trader_data_logger.handlers):
    ...
    trader_logger.info("Starting TraderAgent example with real API calls, demonstrating different modes...")

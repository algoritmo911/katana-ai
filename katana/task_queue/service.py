...
class TaskQueueService:
    def __init__(self, broker: AbstractBroker, task_executors: Dict[str, TaskExecutor]):
        ...
        if not isinstance(task_executors, dict) or not all(
            ...
            raise TypeError("task_executors must be a dictionary of string keys to callable async functions.")
    ...
    async def add_task(
        ...
        task_id: Optional[uuid.UUID] = None # Allow specifying ID for idempotent retries etc.
    ) -> Task:
        """
        ...
        if name not in self.task_executors:
            ...
            raise ValueError(f"Task name '{name}' not found in registered task executors.")
        ...
        logger.info(f"Task {task.id} (Name: {task.name}, Priority: {task.priority}, Scheduled: {task.scheduled_at}) enqueued.")
        return task
    ...
    async def _execute_task(self, task: Task) -> None:
        ...
        if not executor:
            logger.error(f"No executor found for task name '{task.name}' (ID: {task.id}). Marking as FAILED.")
            await self.broker.mark_failed(task.id)
            return

        logger.info(f"Starting execution of task {task.id} (Name: {task.name}).")
        try:
            await executor(task) # The executor function is responsible for its own logic
            await self.broker.mark_complete(task.id)
            logger.info(f"Task {task.id} (Name: {task.name}) completed successfully.")
        except Exception as e:
            logger.error(f"Error executing task {task.id} (Name: {task.name}): {e}", exc_info=True)
            await self.broker.mark_failed(task.id)
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if dequeued_task:
                    ...
                    if current_task_state and current_task_state.status == TaskStatus.PENDING:
                        ...
                        updated = await self.broker.update_task_status(dequeued_task.id, TaskStatus.PROCESSING)
                        if updated:
                            task_to_process = await self.broker.get_task(dequeued_task.id) # Get the version with PROCESSING status
                            logger.info(f"Worker {worker_id} picked up task {task_to_process.id} (Name: {task_to_process.name}).")
                        else:
                            logger.warning(f"Worker {worker_id}: Failed to update task {dequeued_task.id} to PROCESSING. It might have been processed or changed by another worker.")
                            continue # Try to get another task
                    elif current_task_state:
                        logger.info(f"Worker {worker_id}: Dequeued task {dequeued_task.id} but its status is {current_task_state.status}. Skipping.")
                        continue
                    else:
                        logger.warning(f"Worker {worker_id}: Dequeued task ID {dequeued_task.id} but task data not found in broker. Skipping.")
                        continue
                ...
                logger.info(f"Worker {worker_id} received cancellation request. Shutting down...")
    ...
    def start_workers(self, num_workers: int, poll_interval: float = 0.1) -> List[asyncio.Task]:
        """
        ...
        for i in range(num_workers):
            ...
            task = asyncio.create_task(self.worker(worker_id=i+1, poll_interval=poll_interval))
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("Shutdown signal sent. Workers will stop after their current task or next poll.")


# Example Task Executor Functions (to be defined by the user of the service)
async def example_task_executor_log(task: Task):
    logger.info(f"Executing example_task_executor_log for task {task.id}: Name='{task.name}', Payload='{task.payload}'")
    # Simulate work
...
async def another_example_executor(task: Task):
    logger.info(f"Executing another_example_executor for task {task.id}: Name='{task.name}', Payload='{task.payload}'")
...
async def main_service_example():
    ...
    # 1. Setup Broker
    from katana.task_queue.broker import InMemoryBroker # Assuming this is in the same directory for example
    ...
    # 5. Add some tasks
    task1 = await service.add_task(name="log_message", payload={"message": "Hello from Task 1", "user_id": 123}, priority=1)
    task2 = await service.add_task(name="short_work", payload={"data": "Some data for short work"}, priority=0)
    task3 = await service.add_task(name="log_message", payload={"message": "Delayed critical task", "critical": True}, priority=0, delay_seconds=3)
    task4_fail = await service.add_task(name="log_message", payload={"message": "This task will fail", "force_fail": True}, priority=2)
    ...
    await service.add_task(name="short_work", payload={"data": "Another short work"}, priority=1)
    await service.add_task(name="log_message", payload={"message": "Low priority task"}, priority=10)
    ...
    logger.info("\nFinal Task Statuses:")
    for task_obj in [task1, task2, task3, task4_fail]: # Add newly added tasks if you want to check them
        final_status_task = await broker.get_task(task_obj.id)
        if final_status_task:
            logger.info(f"Task {final_status_task.name} (ID: {final_status_task.id}): {final_status_task.status}")
        else:
            logger.info(f"Task ID {task_obj.id} not found in broker (unexpected).")

    # Check a task that was added later
    task5_check = await broker.get_task((await broker.dequeue()).id) # This is a bit of a hack to get a recent task ID

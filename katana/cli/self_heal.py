...
def setup_daemon_logging():
    ...
    try:
        ...
        sys.stderr.write("Warning: os.devnull not found, stdin may not be fully detached.\n")
    except OSError as e:
    ...
    try:
        ...
        sys.stderr.write(f"Fatal: Could not open log file {LOG_FILE} for writing: {e}\n")
        sys.exit(1)
    ...
    try:
        if os.path.exists(PID_FILE):
            ...
            if current_pid_in_file is None or current_pid_in_file == os.getpid():
                ...
                print(f"PID file {PID_FILE} (PID {current_pid_in_file}) not removed by process {os.getpid()} as it seems to belong to another process.")
    except IOError as e:
...
def read_pid_file():
    try:
        ...
        try:
            corrupt_path = PID_FILE + ".corrupt_" + datetime.now().strftime("%Y%m%d%H%M%S")
...
def _update_status_file_internal(status_dict_to_write, is_crash_update=False):
    ...
    try:
        ...
        if not is_crash_update: # Avoid logging during critical shutdown/crash status update
        ...
        if not is_crash_update: # Avoid modifying dict if it's already a crash dict
            status_dict_to_write.setdefault("warnings", []).append([time.time(), f"Failed to write status to file: {e_status_write}"])
...
def daemon_main_loop():
    ...
    print(f"Daemon process (PID: {os.getpid()}) main loop started. Current status: {health_status_global.get('status')}")
    ...
    health_status_global.setdefault("started_at", time.time()) # This remains from the very first start
    ...
    health_status_global["current_loop_restarted_at"] = time.time() # Timestamp for this specific main_loop instance
...
def _shutdown_daemon(signal_name_or_reason):
    global health_status_global
    print(f"Shutdown initiated by '{signal_name_or_reason}'. Daemon (PID: {os.getpid()}) initiating shutdown...")

    health_status_global["status"] = f"shutting_down ({signal_name_or_reason})"
    health_status_global["last_updated"] = time.time()
    health_status_global.setdefault("warnings", []).append([time.time(), f"Shutdown initiated by {signal_name_or_reason}."])
...
def sighup_handler(signum, frame):
    global health_status_global
    print(f"SIGHUP received. Daemon (PID: {os.getpid()}) reloading configuration (simulated)...")

    health_status_global["status"] = "reloading_config (simulated)"
    health_status_global["last_updated"] = time.time()
    health_status_global.setdefault("warnings", []).append([time.time(), "SIGHUP received, configuration reload simulated."])
    _update_status_file_internal(health_status_global)
...
def run_daemon(args):
    ...
    if pid_from_file:
        try:
            ...
            print(f"Error: Daemon already running with PID {pid_from_file} (from {PID_FILE}).")
            sys.exit(1)
        except ProcessLookupError:
            print(f"Warning: Stale PID file found ({PID_FILE} for PID {pid_from_file}, process not running). Removing it.")
            try:
                os.remove(PID_FILE)
                print(f"Stale PID file {PID_FILE} removed successfully.")
            except OSError as e_remove:
                print(f"Error removing stale PID file {PID_FILE}: {e_remove}. Please remove it manually.", file=sys.stderr)
                sys.exit(1)
        except PermissionError:
            print(f"Error: Daemon process with PID {pid_from_file} is running, but you don't have permission to signal it.", file=sys.stderr)
            sys.exit(1)
        except Exception as e_kill:
            print(f"Error checking status of existing PID {pid_from_file}: {e_kill}", file=sys.stderr)
            sys.exit(1)
            ...
            try:
                ...
                if wait_pid == child_pid1:
                    if os.WIFEXITED(status) and os.WEXITSTATUS(status) == 0:
                        print("Daemon process setup initiated successfully in background.")
                    else: # Intermediate process exited with error
                        print(f"Error: Daemon setup process (PID: {child_pid1}) exited with status {os.WEXITSTATUS(status)}.", file=sys.stderr)
                        print("Daemon may not have started. Check logs if any (e.g. /tmp/katana_self_heal.log).", file=sys.stderr)
                        sys.exit(1)
                # else: intermediate process still running, this is fine.
            except ChildProcessError:
                 print("Warning: No child process found after first fork (unexpected), but continuing.", file=sys.stderr)
            except OSError as e_wait:
                 print(f"Warning: Error waiting for first fork child: {e_wait}", file=sys.stderr)
            sys.exit(0)
    ...
    print(f"Daemon process starting. PID: {os.getpid()}, PPID: {os.getppid()}, SID: {os.getsid(0)}")
    ...
    while restart_attempts_done <= MAX_RESTART_ATTEMPTS:
        ...
        if restart_attempts_done > 0:
            print(f"--- Daemon Main Loop: Attempting restart {restart_attempts_done}/{MAX_RESTART_ATTEMPTS} after {RESTART_DELAY_SECONDS} seconds ---")
            health_status_global["status"] = f"restarting_main_loop (attempt {restart_attempts_done})"
            _update_status_file_internal(health_status_global)
        ...
        try:
            ...
            print(f"SystemExit caught in run_daemon's loop ({e_sysexit.code}). Daemon is exiting.")
            ...
            print(f"ERROR: Unhandled exception in daemon main loop (Crash number {restart_attempts_done}): {e_mainloop}", file=sys.stderr)
            trace_info = traceback.format_exc()
            print(f"Traceback:\n{trace_info}", file=sys.stderr)

            health_status_global["status"] = f"crashed_attempting_restart (after crash {restart_attempts_done})"
            health_status_global["error"] = str(e_mainloop)
            health_status_global["last_updated"] = time.time()
            health_status_global["traceback_snippet"] = trace_info.splitlines()[-5:] # Store last 5 lines
            _update_status_file_internal(health_status_global, is_crash_update=True)
            print("Status file updated to reflect crash and pending restart.")

            if restart_attempts_done > MAX_RESTART_ATTEMPTS:
                print(f"FATAL: Maximum main loop restart attempts ({MAX_RESTART_ATTEMPTS}) reached. Daemon will now exit.")
                health_status_global["status"] = "crashed_max_restarts_exceeded"
                health_status_global["error"] = str(e_mainloop) # Keep last error
                health_status_global["traceback"] = trace_info.splitlines() # Store full traceback for final crash
                _update_status_file_internal(health_status_global, is_crash_update=True)
            ...
            print("Daemon main loop finished without error (returned). This implies a controlled stop.")
...
def get_status(args):
    ...
    if pid_from_file:
        try:
            os.kill(pid_from_file, 0)
            print(f"System reports process with PID {pid_from_file} (from {PID_FILE}) is alive.")
            daemon_is_confirmed_running = True
        except ProcessLookupError:
            print(f"System reports process with PID {pid_from_file} (from {PID_FILE}) is NOT running.")
        except PermissionError:
            print(f"Process with PID {pid_from_file} (from {PID_FILE}) is running, but you lack permissions to send signals.")
            daemon_is_confirmed_running = True
        except OSError as e_kill_check:
            print(f"Error checking daemon process PID {pid_from_file} via kill: {e_kill_check}. PID file might be stale.")
    else:
        print(f"Daemon PID file ({PID_FILE}) not found or invalid. Daemon is likely not running.")

    if not os.path.exists(STATUS_FILE):
        if daemon_is_confirmed_running:
            print(f"Status file {STATUS_FILE} not found, but a process with PID {pid_from_file} is active.")
            print("Daemon might be initializing, status file path misconfigured, or not yet written.")
        else:
            print(f"Status file {STATUS_FILE} not found. No active daemon PID detected.")
        return
    ...
    try:
        ...
        if pid_from_file and reported_pid_in_status and reported_pid_in_status != pid_from_file:
        ...
        non_active_statuses = ["shutting_down", "crashed", "stopped", "crashed_max_restarts_exceeded", "main_loop_returned", "main_loop_completed_normally"]
        if not daemon_is_confirmed_running and current_daemon_status_from_file not in non_active_statuses and current_daemon_status_from_file != "unknown":
        ...
        elif daemon_is_confirmed_running and any(s in current_daemon_status_from_file for s in ["crashed", "stopped"]): # Check substrings for crashed states
        ...
        if error_info:
            ...
            print(error_info)
            tb_info = status_data.get("traceback_snippet") or status_data.get("traceback")
            ...
            for item in warnings_list:
                if isinstance(item, list) and len(item) == 2:
                    ...
                    try:
                        ...
                        print(f"- At {warn_dt.strftime('%Y-%m-%d %H:%M:%S')}: {warn_msg}")
        ...
        print(f"Error: Could not parse status file {STATUS_FILE}. It might be corrupted.")
        if daemon_is_confirmed_running:

...
class KatanaCore:
    def __init__(self, core_dir_path_str="."):
        ...
        logger.info(f"KatanaCore initialized. Operational directory: {self.core_dir}")
        # logger.info(f"Commands loaded: {list(self.commands.keys())}") # Can be verbose
    ...
    def _ensure_files_exist(self):
        ...
        for file_path, default_content in default_files_content.items():
            if not file_path.exists():
                logger.warning(f"File {file_path} not found. Creating with default content.")
    ...
    def load_commands(self):
        ...
        try:
            ...
            logger.info(f"Commands loaded successfully from {self.commands_file_path}.")
        except json.JSONDecodeError as e:
            logger.error(f"Error decoding JSON from {self.commands_file_path}: {e}. Using empty command set.")
            self.commands = {}
        except FileNotFoundError: # Should be handled by _ensure_files_exist, but as fallback:
            logger.error(f"Commands file {self.commands_file_path} not found during load. Initializing empty.")
            self.commands = {}
            self._save_json(self.commands_file_path, self.commands) # Try to create it
        except Exception as e:
            logger.error(f"Failed to load commands from {self.commands_file_path}: {e}. Using empty command set.")
            self.commands = {}
    ...
    def save_status(self, new_status_data=None):
        ...
        if self._save_json(self.status_file_path, self.status):
            logger.info(f"Status saved to {self.status_file_path}")
        else:
            logger.error(f"Failed to save status to {self.status_file_path}")
    ...
    def load_status(self):
        ...
        try:
            ...
            logger.info(f"Status loaded from {self.status_file_path}")
        except json.JSONDecodeError as e:
            logger.error(f"Error decoding JSON from {self.status_file_path}: {e}. Using default status.")
            ...
            logger.error(f"Status file {self.status_file_path} not found during load. Initializing default.")
            ...
            logger.error(f"Failed to load status from {self.status_file_path}: {e}. Using default status.")
    ...
    def load_memory(self):
        ...
        try:
            ...
            logger.error(f"Error decoding JSON from {self.memory_file_path}: {e}. Initializing empty memory.")
            self.memory = {}
        except FileNotFoundError: # Should be handled by _ensure_files_exist
            logger.error(f"Memory file {self.memory_file_path} not found during load. Initializing empty.")
            self.memory = {}
            self._save_json(self.memory_file_path, self.memory) # Try to create it
        except Exception as e:
            logger.error(f"Failed to load memory from {self.memory_file_path}: {e}. Initializing empty memory.")
            self.memory = {}

    def save_memory(self):
        """Saves the current memory to the memory.json file."""
        if self._save_json(self.memory_file_path, self.memory):
            logger.info(f"Memory saved to {self.memory_file_path}")
        else:
            logger.error(f"Failed to save memory to {self.memory_file_path}")


    def run(self):
        """Main run loop for the Katana agent's CLI."""
        logger.info("⚔️ KatanaCore activated. Type 'exit' or 'quit' to stop. Waiting for command...")
        ...
        while True:
            try:
                ...
                if cmd_key in ['exit', 'quit']:
                    logger.info("Exit command received. Shutting down KatanaCore.")
                ...
                if cmd_key in self.commands:
                    ...
                    logger.info(f"Executing command '{cmd_key}': {command_to_execute}")
                    ...
                    if exit_code == 0:
                        logger.info(f"Command '{cmd_key}' executed successfully.")
                        self.save_status({"last_command_result": "success", "status": "idle_after_command"})
                    else:
                        logger.error(f"Command '{cmd_key}' failed with exit code: {exit_code}.")
                    ...
                    if len(parts) == 3:
                        ...
                        self.save_memory()
                        logger.info(f"Memorized: {{mem_key}} = {{mem_value}}") # Escaped for script
                    else:
                        logger.warning("Usage: remember <key> <value>")
                    ...
                    if len(parts) == 2:
                        ...
                        print(f"Recalled {{mem_key}}: {{recalled_value}}") # CLI output, keep as print
                        logger.info(f"Recalled: {{mem_key}}")
                    else:
                        logger.warning("Usage: recall <key>")

                elif cmd_key == "status":
                    print(f"Current Status: {json.dumps(self.status, indent=2)}") # CLI output, keep as print
                    logger.info("Displayed current status.")

                else:
                    logger.warning(f"Unknown command: '{cmd_input}'")
                    print(f"❌ Unknown command. Available: {list(self.commands.keys())} or 'remember/recall <key> <value>', 'status', 'exit'.") # CLI output, keep as print
...
if __name__ == '__main__':
    ...
    # kc.run()
    # Note: If setup_logging() hasn't been called by an entry point, these logs might not appear as configured.
    logger.info("KatanaCore class defined. To run, instantiate and call .run()")
    logger.info("Example: kc = KatanaCore('path/to/katana_core_data_dir'); kc.run()")

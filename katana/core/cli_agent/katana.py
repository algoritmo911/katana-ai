...
class KatanaCore:
    ...
    def load_commands(self):
        ...
        try:
            ...
            logger.error(f"Failed to load commands from {self.commands_file_path}: {e}. Using empty command set.", exc_info=True)
            self.commands = {}
    ...
    def load_status(self):
        ...
        try:
            ...
            logger.error(f"Failed to load status from {self.status_file_path}: {e}. Using default status.", exc_info=True)
    ...
    def load_memory(self):
        ...
        try:
            ...
            logger.error(f"Failed to load memory from {self.memory_file_path}: {e}. Initializing empty memory.", exc_info=True)
            self.memory = {}
            ...
            try:
                ...
                if cmd_key in self.commands:
                    ...
                    if len(parts) == 3:
                        ...
                        self.save_memory()
                        logger.info(f"Memorized: {mem_key} = {mem_value}")
                    else:
                    ...
                    if len(parts) == 2:
                        ...
                        print(f"Recalled {mem_key}: {recalled_value}") # Direct user output
                        logger.info(f"Recalled: {mem_key}")
                    else:
                        logger.warning("Usage: recall <key>")

                elif cmd_key == "status":
                    print(f"Current Status: {json.dumps(self.status, indent=2)}") # Direct user output
                    logger.info("Displayed current status.")

                else:
                    logger.warning(f"Unknown command: '{cmd_input}'")
                    print(f"❌ Unknown command. Available: {list(self.commands.keys())} or 'remember/recall <key> <value>', 'status', 'exit'.") # Direct user output

            except KeyboardInterrupt:
                logger.info("KeyboardInterrupt received. Shutting down KatanaCore gracefully...")

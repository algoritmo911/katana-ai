...
class TestKatanaAgentLogging(unittest.TestCase):
    ...
    def test_agent_initialization_logging(self, mock_logger):
        ...
        mock_logger.info.assert_called_with("KatanaAgent '%s' initialized.", "TestAgent")
    ...
    def test_perform_action_logging(self, mock_logger):
        ...
        mock_logger.debug.assert_any_call("Agent '%s' attempting to perform action: %s", "ActionLogger", "test action")
        mock_logger.info.assert_any_call("Agent '%s' successfully performed action: %s", "ActionLogger", "test action")

        # Failed action (empty description)
        agent.perform_action("")
        mock_logger.debug.assert_any_call("Agent '%s' attempting to perform action: %s", "ActionLogger", "")
        mock_logger.error.assert_called_with("No action description provided to agent '%s'.", "ActionLogger")
    ...
    def test_report_status_logging(self, mock_logger):
        ...
        agent.report_status()
        mock_logger.warning.assert_called_with("Agent '%s' reporting status: All systems nominal (example warning).", "StatusReporter")

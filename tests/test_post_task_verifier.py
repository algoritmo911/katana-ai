...
class TestPostTaskVerifier(unittest.TestCase):
    ...
    def setUp(self):
        ...
        post_task_verifier.DUMMY_TASK_EXPECTATIONS = (
            ORIGINAL_DUMMY_TASK_EXPECTATIONS.copy()
        )
        ...
        if "create_config_file" in post_task_verifier.DUMMY_TASK_EXPECTATIONS:
            post_task_verifier.DUMMY_TASK_EXPECTATIONS["create_config_file"][
                "filepath"
            ] = self.config_env_path
        if "run_migrations" in post_task_verifier.DUMMY_TASK_EXPECTATIONS:
            post_task_verifier.DUMMY_TASK_EXPECTATIONS["run_migrations"][
                "log_file"
            ] = self.migrations_log_path

        # A specific task for testing log failure due to missing keyword
        post_task_verifier.DUMMY_TASK_EXPECTATIONS[
            "run_migrations_keyword_miss_test"
        ] = {
            "type": "log_check",
    ...
    def test_verify_task_file_check_failure(self):
        ...
        self.assertEqual(
            result["reason"],
            post_task_verifier.DUMMY_TASK_EXPECTATIONS[task_id]["failure_reason"],
        )
    ...
    def test_verify_task_file_check_success(self):
        ...
        self.assertEqual(
            result["message"],
            post_task_verifier.DUMMY_TASK_EXPECTATIONS[task_id]["success_message"],
        )
    ...
    def test_verify_task_log_check_simulated_creation_and_success(self):
        ...
        self.assertTrue(
            os.path.exists(self.migrations_log_path)
    ...
    def test_verify_task_port_check_simulated_success(self):
        ...
        if (
            task_id not in post_task_verifier.DUMMY_TASK_EXPECTATIONS
            or post_task_verifier.DUMMY_TASK_EXPECTATIONS[task_id]["type"]
            != "port_check"
        ):
            self.skipTest(
                f"Task {task_id} not configured for port_check in DUMMY_TASK_EXPECTATIONS"
            )
    ...
    def test_verify_task_unknown_task_default_success(self):
        ...
        self.assertTrue(
            result["reason"].startswith(
                f"Задача '{task_id}' не имеет специфических критериев"
            )
        )
    ...
    def test_write_katana_result_ensure_ascii_false(self):
        test_result_data = {
            "task_id": "test_cyrillic",
            "status": "failure",
            "reason": "Ошибка: файл не найден",
        }

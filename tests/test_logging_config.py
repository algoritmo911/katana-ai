...
class TestLoggingConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if os.path.exists(LOG_DIR) and not os.listdir(LOG_DIR): # Only remove if empty
            os.rmdir(LOG_DIR)
        elif os.path.exists(LOG_DIR) and os.listdir(LOG_DIR) and os.path.exists(LOG_FILE): # If log file was the only thing
    ...
    def test_setup_logging_basic_configuration(self, mock_stream_handler, mock_file_handler, mock_makedirs):
        ...
        mock_file_handler_instance.setFormatter.side_effect = lambda f: setattr(mock_file_handler_instance, 'formatter', f)

        mock_stream_handler_instance = mock_stream_handler.return_value
        mock_stream_handler_instance.level = logging.DEBUG
        # Make setFormatter actually set the formatter on the mock instance
        mock_stream_handler_instance.setFormatter.side_effect = lambda f: setattr(mock_stream_handler_instance, 'formatter', f)

...
class TestCoinbaseAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(module)s - %(message)s'))
    ...
    def test_get_spot_price_success(self, mock_get):
        ...
        self.assertIn("Successfully fetched spot price for BTC-USD: 50000.75", log_content)
    ...
    def test_get_spot_price_http_error(self, mock_get):
        ...
        self.assertIn("HTTP error occurred while fetching price for INVALID-PAIR", log_content)
    ...
    def test_get_spot_price_connection_error(self, mock_get):
        ...
        with open(COINBASE_LOG_FILE, 'r') as f:
            log_content = f.read()
        self.assertIn("Connection error occurred while fetching price for BTC-USD: Failed to connect", log_content)
    ...
    def test_get_spot_price_timeout_error(self, mock_get):
        ...
        with open(COINBASE_LOG_FILE, 'r') as f:
            log_content = f.read()
        self.assertIn("Timeout error occurred while fetching price for BTC-USD: Request timed out", log_content)
    ...
    def test_get_spot_price_invalid_price_format_in_response(self, mock_get):
        ...
        with open(COINBASE_LOG_FILE, 'r') as f:
            log_content = f.read()
        self.assertIn("Error converting price to float for BTC-USD. Price string: 'not_a_number'", log_content)

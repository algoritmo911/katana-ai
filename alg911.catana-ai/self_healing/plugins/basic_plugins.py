...
from ..self_healing_logger import logger # Assuming logger is accessible from parent package
...
class HttpAvailabilityMonitor(MonitoringPlugin):
    ...
    def check_health(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not url:
            return {"status": "error", "error_message": "URL not configured for HttpAvailabilityMonitor"}

        logger.debug(f"{self.get_name()}: Checking {method} {url} (expecting {expected_status_code})")
        result_data: Dict[str, Any] = {"url": url, "method": method, "expected_status_code": expected_status_code}

        try:
            response = requests.request(method, url, headers=headers, json=payload, timeout=timeout_seconds, allow_redirects=True)
            ...
            if response.status_code == expected_status_code:
                ...
                result_data["details"] = f"Service responded with {response.status_code} as expected."
            else:
                result_data["status"] = "error"
                result_data["error_message"] = f"Unexpected status code: {response.status_code}. Expected: {expected_status_code}."
                try:
                    result_data["response_content_snippet"] = response.text[:200] # First 200 chars
                except Exception:
                    result_data["response_content_snippet"] = "Could not retrieve response content."
            logger.debug(f"{self.get_name()}: Check for {url} result: {result_data['status']}")
        except requests.exceptions.Timeout:
            result_data["status"] = "error"
            result_data["error_message"] = f"Request timed out after {timeout_seconds} seconds."
            ...
            logger.error(f"{self.get_name()}: Unexpected error checking {url}: {e}", exc_info=True)
...
class ProcessRunningMonitor(MonitoringPlugin):
    ...
    def check_health(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result_data: Dict[str, Any] = {"process_name_pattern": process_name_pattern, "pid_file": pid_file}

        if not process_name_pattern and not pid_file:
            result_data["status"] = "error"
            result_data["error_message"] = "Either 'process_name_pattern' or 'pid_file' must be configured."
            return result_data
        ...
        if pid_file:
            ...
            try:
                if os.path.exists(pid_file):
                    with open(pid_file, "r") as f:
                        ...
                        if pid_str.isdigit():
                            ...
                            if os.path.exists(f"/proc/{pid}"): # Linux specific, adjust for other OS
                                pids_found.append(pid)
                                result_data["pid_from_file"] = pid
                            else:
                                result_data["status"] = "error"
                                result_data["error_message"] = f"PID {pid} from file {pid_file} does not exist."
                        else:
                            result_data["status"] = "error"
                            result_data["error_message"] = f"Content of PID file {pid_file} is not a valid PID: {pid_str}"
                else:
                    result_data["status"] = "error"
                ...
                logger.warning(f"{self.get_name()}: Error with PID file {pid_file}: {e}")

        if process_name_pattern and not pids_found: # Only check by pattern if PID file didn't yield a running process or wasn't used
            logger.debug(f"{self.get_name()}: Checking for process pattern '{process_name_pattern}'")
            try:
                ...
                if process.returncode == 0:
                    ...
                    result_data["status"] = "error"
                    result_data["error_message"] = f"Error checking process with pgrep: {err_msg}"
                    logger.warning(f"{self.get_name()}: pgrep error for '{process_name_pattern}': {err_msg}")

            except FileNotFoundError: # pgrep not found
                 result_data["status"] = "error"
                 result_data["error_message"] = "pgrep command not found. Cannot check process by pattern."
                 logger.error(f"{self.get_name()}: pgrep not found.")
            except subprocess.TimeoutExpired:
                 result_data["status"] = "error"
                 result_data["error_message"] = "pgrep command timed out."
                 logger.warning(f"{self.get_name()}: pgrep timeout for '{process_name_pattern}'.")
            except Exception as e:
                 result_data["status"] = "error"
                 result_data["error_message"] = f"Exception checking process pattern: {e}"
                 logger.error(f"{self.get_name()}: Exception with pgrep for '{process_name_pattern}': {e}", exc_info=True)
        ...
        if pids_found:
            ...
            result_data["status"] = "error" # Changed from "warning" to "error" as it's an issue if not found
            result_data["error_message"] = "Process not found."
            result_data["pids"] = []

        logger.debug(f"{self.get_name()}: Check for process '{process_name_pattern or pid_file}' result: {result_data.get('status')}")
        return result_data
...
class LogFileMonitor(MonitoringPlugin):
    ...
    _last_error_timestamps: Dict[str, float] = {} # To avoid re-flagging same error burst
    ...
    def check_health(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not os.path.exists(log_file_path):
            return {"status": "warning", "log_file": log_file_path, "details": "Log file does not exist."}

        logger.debug(f"{self.get_name()}: Checking log file '{log_file_path}' for patterns: {error_patterns}")
        ...
        try:
            ...
            if file_size < current_pos: # Log file rotated or truncated
                current_pos = 0
                logger.info(f"{self.get_name()}: Log file '{log_file_path}' appears to have rotated/truncated. Resetting read position.")

            lines_read_count = 0
            if file_size > current_pos:
                with open(log_file_path, "r", encoding="utf-8", errors="ignore") as f:
                    f.seek(current_pos)
                    for line_number, line_content in enumerate(f, start=int(current_pos / 80)): # Approx line number
                        lines_read_count += 1
                        for pattern_index, pattern in enumerate(error_patterns):
                            # Basic string matching, for regex use re.search
                            if pattern in line_content: # Use `re.search(pattern, line_content)` for regex
                                found_patterns_details.append({
                                    "pattern": pattern,
                                    "line_number": line_number, # This is an approximation
                                    "line_content": line_content.strip()[:200] # Snippet
                                })
            ...
            if found_patterns_details:
                ...
                if time.time() - last_err_ts > debounce_period_seconds:
                    ...
                    logger.warning(f"{self.get_name()}: Found {len(found_patterns_details)} pattern matches in '{log_file_path}'. Lines checked: {lines_read_count}")
                    return {
                        "status": "warning",
                        "log_file": log_file_path,
                        "found_patterns_count": len(found_patterns_details),
                        "details_snippet": found_patterns_details[:3] # First 3 matches
                    }
                else:
                    logger.debug(f"{self.get_name()}: Patterns found in '{log_file_path}' but within debounce period. Suppressing new warning status.")
                    return {"status": "ok", "log_file": log_file_path, "details": "Patterns found but debounced."}


            logger.debug(f"{self.get_name()}: No new critical patterns found in '{log_file_path}'. Lines checked: {lines_read_count}")
            return {"status": "ok", "log_file": log_file_path, "details": f"No new error patterns found. Lines checked: {lines_read_count}"}

        except Exception as e:
            logger.error(f"{self.get_name()}: Error reading log file '{log_file_path}': {e}", exc_info=True)
            return {"status": "error", "log_file": log_file_path, "error_message": str(e)}
...
class HttpStatusIssueDiagnoser(DiagnosticPlugin):
    ...
    def diagnose(self, monitor_data_list: List[Dict[str, Any]], config: Dict[str, Any]) -> List[Dict[str, Any]]:
        issues = []
        critical_status_codes = config.get("critical_status_codes", [500, 502, 503, 504])
        ...
        for data_point in monitor_data_list:
            ...
            if data_point.get("status") == "error":
                ...
                details = data_point.get("error_message", "HTTP monitor reported an error.")

            http_status_code = data_point.get("http_status_code")
            if http_status_code is not None:
                expected_code = data_point.get("expected_status_code")
                if http_status_code != expected_code : # Already an issue if codes don't match
                    is_issue = True
                    details = f"Expected status {expected_code} but got {http_status_code}. " + data_point.get("error_message", "")
                ...
                elif http_status_code in critical_status_codes and http_status_code == expected_code:
                     is_issue = True # Still log it as an issue if it's inherently critical
                     severity = "critical"
            ...
            if is_issue:
                issues.append({
                    ...
                    "source_data_snippet": {k:v for k,v in data_point.items() if k not in ['target_id', 'monitor_name']}
                })
        if issues:
...
class ProcessNotRunningDiagnoser(DiagnosticPlugin):
    ...
    def diagnose(self, monitor_data_list: List[Dict[str, Any]], config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for data_point in monitor_data_list:
            ...
            if data_point.get("status") == "error" and "Process not found" in data_point.get("error_message",""):
                issues.append({
            ...
            elif data_point.get("status") == "ok" and not data_point.get("pids"): # Should not happen if status is ok
                 issues.append({
...
class LogPatternDiagnoser(DiagnosticPlugin):
    ...
    def diagnose(self, monitor_data_list: List[Dict[str, Any]], config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for data_point in monitor_data_list:
            ...
            if data_point.get("status") == "warning": # LogFileMonitor sets to 'warning' if patterns found
                ...
                if num_found >= critical_pattern_threshold:
                    issues.append({
                        ...
                        "patterns_found_details": data_point.get('details_snippet') # Or full list if needed
                    })
        if issues:
            logger.debug(f"{self.get_name()} diagnosed {len(issues)} log pattern issues.")
        return issues
...
class ServiceRestartRecovery(RecoveryPlugin):
    ...
    def can_recover(self, issue: Dict[str, Any]) -> bool:
        ...
        if issue.get("issue_type") in recoverable_issue_types:
            ...
            if "HTTP" in issue.get("issue_type","") and issue.get("severity") != "critical":
                return False
            return True
        return False
    ...
    def attempt_recovery(self, issue: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not service_name_to_operate:
            return {"status": "failed", "action_taken": "restart_attempt", "error_message": "Service name for restart not configured or derivable from issue target_id."}
        ...
        restart_command_template = config.get("restart_command_template", "echo Simulating: sudo systemctl restart {service_name}")
        ...
        if direct_command:
            ...
            command_to_run = restart_command_template.format(service_name=service_name_to_operate)

        logger.info(f"{self.get_name()}: Attempting to restart service '{service_name_to_operate}' with command: {command_to_run}")
        ...
        try:
            ...
            if process.returncode == 0:
                logger.info(f"{self.get_name()}: Service '{service_name_to_operate}' restart command executed successfully. Output: {process.stdout.strip()}")
                ...
                logger.error(f"{self.get_name()}: Service '{service_name_to_operate}' restart command failed. RC: {process.returncode}, Error: {process.stderr.strip()}")
            ...
            logger.error(f"{self.get_name()}: Timeout restarting service {service_name_to_operate} with command: {command_to_run}")
            return {"status": "failed", "action_taken": f"Restart {service_name_to_operate}", "error_message": "Restart command timed out."}
        except FileNotFoundError: # If the command itself (e.g. systemctl) is not found
            logger.error(f"{self.get_name()}: Restart command or script not found: {command_to_run.split()[0]}")
            return {"status": "failed", "action_taken": f"Restart {service_name_to_operate}", "error_message": f"Command not found: {command_to_run.split()[0]}"}
        except Exception as e:
            logger.error(f"{self.get_name()}: Error restarting service {service_name_to_operate} with command '{command_to_run}': {e}", exc_info=True)
            return {"status": "failed", "action_taken": f"Restart {service_name_to_operate}", "error_message": str(e)}


# Example of a placeholder for a more complex recovery plugin
class ConfigurationRollbackRecovery(RecoveryPlugin):
    def get_name(self) -> str: return "ConfigurationRollbackRecovery"
    def can_recover(self, issue: Dict[str, Any]) -> bool:
        # Define which issues might be solved by a config rollback
        return issue.get("issue_type") == "INVALID_CONFIG_DETECTED" # Assuming such an issue type exists

    def attempt_recovery(self, issue: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
        logger.info(f"{self.get_name()}: Placeholder - Simulating configuration rollback for {issue.get('target_id')}")
        ...
        return {"status": "skipped", "action_taken": "Configuration rollback (simulated)", "details": "This is a placeholder plugin."}
...
if __name__ == "__main__":
    ...
    non_existent_health = http_monitor.check_health({"url": "http://thissitedefinitelydoesnotexist12345.com"})
    ...
    if os.name == 'posix': # Linux, macOS
        ...
        logger.info("Skipping ProcessRunningMonitor tests as OS is not POSIX or pgrep might not be available.")
    ...
    log_check_no_pattern = log_monitor.check_health({"log_file_path": dummy_log_path, "error_patterns": ["NonExistentPattern"]})
    logger.info(f"Log Check (no matching pattern): {log_check_no_pattern}")
    log_check_with_pattern = log_monitor.check_health({"log_file_path": dummy_log_path, "error_patterns": ["ERROR", "CRITICAL"]})
    ...
    if log_check_with_pattern.get("status") == "warning":
        log_check_debounced = log_monitor.check_health({"log_file_path": dummy_log_path, "error_patterns": ["ERROR", "CRITICAL"]})
        logger.info(f"Log Check (debounced?): {log_check_debounced}")

    if os.path.exists(dummy_log_path): os.remove(dummy_log_path)
    ...
    http_monitor_outputs = [
        ...
        {"target_id": "service1_api", "monitor_name": "HttpAvailabilityMonitor", "status": "error", "error_message": "Connection refused", "url": "http://localhost:1234"},
        # Case 2: Unexpected 503 status code
        {"target_id": "service2_payment", "monitor_name": "HttpAvailabilityMonitor", "status": "error", "http_status_code": 503, "expected_status_code": 200, "url": "http://service2/pay"},
        # Case 3: Expected 200, got 200 (OK)
        {"target_id": "service3_user", "monitor_name": "HttpAvailabilityMonitor", "status": "ok", "http_status_code": 200, "expected_status_code": 200, "url": "http://service3/user"},
        # Case 4: Expected 200, got 404 (Warning)
        {"target_id": "service4_docs", "monitor_name": "HttpAvailabilityMonitor", "status": "error", "http_status_code": 404, "expected_status_code": 200, "url": "http://service4/docs/nonexistent"}
    ]
    http_issues = http_diagnoser.diagnose(http_monitor_outputs, {"critical_status_codes": [500,503]})
    logger.info(f"HTTP Diagnoser Issues ({len(http_issues)} found):")
    for issue in http_issues: logger.info(f"  - {issue}")


    # Test ServiceRestartRecovery
    restart_recovery = ServiceRestartRecovery()
    test_issue_http_error = {"issue_type": "HTTP_MONITOR_ERROR", "severity": "critical", "target_id": "test_api_service"}
    test_issue_process_down = {"issue_type": "PROCESS_NOT_RUNNING", "severity": "critical", "target_id": "test_worker_process"}

    can_recover_http = restart_recovery.can_recover(test_issue_http_error)
    logger.info(f"Can ServiceRestartRecovery handle HTTP_MONITOR_ERROR? {can_recover_http}")
    if can_recover_http:
        # Config for the plugin instance (service_name might come from MONITORED_TARGETS config in real scenario)
        recovery_config_api = {"service_name": "nginx", "restart_command_template": "echo Simulating: sudo systemctl restart {service_name}"} # Use a common service for simulation
        recovery_result_api = restart_recovery.attempt_recovery(test_issue_http_error, recovery_config_api)
    ...
    logger.info(f"Can ServiceRestartRecovery handle PROCESS_NOT_RUNNING? {can_recover_process}")
    if can_recover_process:
        recovery_config_process = {"service_name": "my_custom_worker", "direct_restart_command": "echo Simulating: /opt/scripts/restart_my_worker.sh"}
        recovery_result_process = restart_recovery.attempt_recovery(test_issue_process_down, recovery_config_process)

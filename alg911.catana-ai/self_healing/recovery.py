...
class RecoveryManager:
    ...
    def _load_plugin_class(self, module_name: str, class_name: str) -> Type[RecoveryPlugin] | None:
        ...
        try:
            ...
            if not issubclass(plugin_class, RecoveryPlugin):
                logger.error(f"Plugin class {class_name} from {module_name} does not inherit from RecoveryPlugin.")
                return None
            return plugin_class
        except ImportError:
            logger.error(f"Failed to import module {module_name} for recovery plugin {class_name}.")
            return None
        except AttributeError:
            logger.error(f"Class {class_name} not found in module {module_name} for recovery.")
            return None
        except Exception as e:
            logger.error(f"Unexpected error loading recovery plugin {module_name}.{class_name}: {e}")
            return None
        ...
        for target_id, target_config in config.MONITORED_TARGETS.items():
            ...
            for plugin_entry in recovery_plugin_configs:
                ...
                if not plugin_class_name:
                    logger.warning(f"No 'plugin' name specified in recovery_plugins for target '{target_id}'. Skipping entry: {plugin_entry}")
                    continue
        ...
        for plugin_name, plugin_info in unique_plugin_types_to_load.items():
            ...
            if plugin_class:
                try:
                    ...
                    logger.info(f"Successfully loaded and instantiated recovery plugin: {plugin_name}")
                except Exception as e:
                    logger.error(f"Failed to instantiate recovery plugin {plugin_name}: {e}")

        logger.info(f"Loaded {len(self.recovery_plugins)} unique recovery plugin types.")

    def attempt_all_recoveries(self, diagnosed_issues: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Starting recovery attempts for {len(diagnosed_issues)} diagnosed issues...")

        for issue in diagnosed_issues:
            target_id = issue.get("target_id")
            if not target_id:
                logger.warning(f"Issue missing 'target_id', cannot determine recovery path: {issue}")
            ...
            if not target_config or not target_config.get("enabled", False):
                logger.debug(f"Target '{target_id}' for issue '{issue.get('issue_type')}' not configured for recovery or is disabled.")
                # No explicit result added here, as no recovery was attempted due to config
                continue

            recovery_plugin_configs_for_target = target_config.get("recovery_plugins", [])
            if not recovery_plugin_configs_for_target:
                logger.debug(f"No recovery plugins configured for target '{target_id}'. Cannot attempt recovery for issue: {issue.get('issue_type')}")
                continue
            ...
            for plugin_config_entry in recovery_plugin_configs_for_target:
                ...
                if not plugin_name:
                    logger.warning(f"Recovery plugin entry for '{target_id}' is missing 'plugin' name. Skipping.")
                    continue

                recovery_plugin_instance = self.recovery_plugins.get(plugin_name)
                if not recovery_plugin_instance:
                    logger.error(f"Recovery plugin '{plugin_name}' for target '{target_id}' not found or loaded. Skipping recovery with this plugin.")
                    continue

                if recovery_plugin_instance.can_recover(issue):
                    logger.info(f"Plugin '{recovery_plugin_instance.get_name()}' can attempt recovery for issue: {issue.get('issue_type')} on target '{target_id}'.")
                    plugin_specific_config = plugin_config_entry.get("config", {})

                    try:
                        result = recovery_plugin_instance.attempt_recovery(issue, plugin_specific_config)
                        result["issue_type"] = issue.get("issue_type")
                        result["target_id"] = target_id
                        result["recovered_by"] = recovery_plugin_instance.get_name()
                        result["timestamp"] = logger.handlers[0].formatter.formatTime(logger.makeRecord("",0,"","",0,"",(),None,None))
                        all_recovery_results.append(result)
                        logger.info(f"Recovery attempt by '{recovery_plugin_instance.get_name()}' for '{target_id} - {issue.get('issue_type')}': {result.get('status')}")

                        if result.get("status") == "success":
                            recovered_by_plugin = True
                            break # Stop trying other recovery plugins for this issue if one succeeded
                    except Exception as e:
                        logger.error(f"Error during recovery attempt for '{target_id} - {issue.get('issue_type')}' with plugin '{recovery_plugin_instance.get_name()}': {e}", exc_info=True)
                        all_recovery_results.append({
                            "status": "error",
                    ...
                    logger.debug(f"Plugin '{recovery_plugin_instance.get_name()}' cannot recover issue: {issue.get('issue_type')} on target '{target_id}'.")

            if not recovered_by_plugin:
                 logger.info(f"No suitable recovery plugin succeeded or was found for issue: {issue.get('issue_type')} on target '{target_id}'.")
        ...
        logger.info(f"Finished recovery attempts. Processed {len(diagnosed_issues)} issues, results: {len(all_recovery_results)}.")
...
if __name__ == "__main__":
    ...
    if not recovery_manager.recovery_plugins:
        logger.warning("No recovery plugins were loaded. Ensure 'basic_plugins.py' is populated with recovery plugins and they are correctly referenced in 'self_healing_config.py'.")
        logger.warning("Example dummy recovery plugins are commented out in the __main__ section of recovery.py.")
    else:
        logger.info(f"Available recovery plugins: {list(recovery_manager.recovery_plugins.keys())}")
        ...
        if recovery_results:
            ...
            logger.info("No recovery actions were attempted or no results returned (check plugin loading and issue matching).")

    print(f"RecoveryManager testing complete. Check logs at: {config.LOG_FILE_PATH if hasattr(config, 'LOG_FILE_PATH') else 'self_healing.log'}")

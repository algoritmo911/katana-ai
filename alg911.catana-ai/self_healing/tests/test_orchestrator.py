...
class TestSelfHealingOrchestrator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        SUT_config.MONITORED_TARGETS = (
            {}
        )  # Start with empty, specific tests will populate
        ...
        self.patch_monitor = patch(
            "self_healing.orchestrator.ServiceMonitor", MagicMock()
        )
        self.patch_diagnoser = patch(
            "self_healing.orchestrator.IssueDiagnoser", MagicMock()
        )
        self.patch_recovery_manager = patch(
            "self_healing.orchestrator.RecoveryManager", MagicMock()
        )
        ...
        self.mock_recovery_manager_instance = (
            self.mock_recovery_manager_class.return_value
        )
    ...
    def test_orchestrator_initialization_enabled(self):
        ...
        self.assertEqual(
            orchestrator.loop_interval_seconds, SUT_config.MAIN_LOOP_INTERVAL_SECONDS
        )

    def test_run_cycle_no_issues_found(self):
        self.mock_monitor_instance.run_all_checks.return_value = {
            "target1": [{"status": "ok"}]
        }
        self.mock_diagnoser_instance.run_diagnostics.return_value = (
            []
        )  # No issues diagnosed
        ...
        self.mock_diagnoser_instance.run_diagnostics.assert_called_once_with(
            {"target1": [{"status": "ok"}]}
        )
    ...
    def test_run_cycle_issue_found_and_recovered(self):
        monitor_output = {
            "server_db": [{"status": "error", "details": "DB Connection Lost"}]
        }
        diagnosed_issue = [
            {
                "target_id": "server_db",
        ...
        self.mock_recovery_manager_instance.attempt_all_recoveries.return_value = (
            recovery_result
        )
        ...
        self.mock_recovery_manager_instance.attempt_all_recoveries.assert_called_once_with(
            diagnosed_issue
        )
        ...
        self.assertEqual(
            orchestrator.active_issues_state[issue_key]["recovery_attempts_count"], 1
        )
    ...
    def test_run_cycle_issue_max_recovery_attempts_reached(self):
        ...
        monitor_output = {
            "web_server": [{"status": "error", "details": "503 Service Unavailable"}]
        }
        diagnosed_issue = [
            {
        ...
        self.assertEqual(
            orchestrator.active_issues_state[issue_key]["recovery_attempts_count"], 1
        )  # Count should not increment further for recovery
    ...
    def test_cleanup_resolved_issues(self):
        ...
        orchestrator.active_issues_state = {
            key1: {
                "first_detected_ts": current_time - 60,
                "last_detected_ts": current_time - 30,
                "recovery_attempts_count": 1,
                "original_issue_details": issue1,
            },
            key2: {
                "first_detected_ts": current_time - 60,
                "last_detected_ts": current_time - 5,
                "recovery_attempts_count": 0,
                "original_issue_details": issue2,
            },
        }
        ...
        SUT_config.MONITORED_TARGETS = (
            {  # Needed for the current _cleanup_resolved_issues logic
        ...
        self.assertNotIn(
            key1, orchestrator.active_issues_state
        )  # service_A's issue should be cleared
        self.assertIn(
            key2, orchestrator.active_issues_state
        )  # service_B's issue should remain
    ...
    def test_cleanup_stale_issues(self):
        ...
        orchestrator.active_issues_state = {
            key_stale: {
                ...
                - 60,  # Last seen 60s ago, > 50s threshold
                "recovery_attempts_count": 1,
        ...
        self.assertNotIn(
            key_stale, orchestrator.active_issues_state
        )  # Stale issue should be cleared

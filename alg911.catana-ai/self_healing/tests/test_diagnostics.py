...
class TestIssueDiagnoser(unittest.TestCase):
    ...
    def test_run_diagnostics_with_mock_plugin(self):
        ...
        mock_issue = {"issue_type": "CPU_HIGH", "severity": "warning", "details": "CPU at 90%"}
        mock_plugin = MockDiagnosticPlugin(name="MyMockDiagnoser", issues_to_return=[mock_issue])
        diagnoser.diagnostic_plugins["MyMockDiagnoser"] = mock_plugin

        target_id = "server_alpha"
        monitor_data_for_target = [{"target_id": target_id, "monitor_name": "CpuMonitor", "cpu_usage": 0.9}]
        all_monitor_data = {target_id: monitor_data_for_target}

        # Configure target in SUT_config to use this diagnostic plugin
        diag_plugin_config_entry = {"plugin": "MyMockDiagnoser", "config": {"custom_param": "value"}}
        SUT_config.MONITORED_TARGETS = {
            target_id: {
                "enabled": True,
                "plugin": "CpuMonitor", "config": {}, # Monitor plugin details don't matter much here
        ...
        self.assertEqual(returned_issue["target_id"], target_id) # Enriched by diagnoser
        ...
        self.assertEqual(mock_plugin.diagnose_called_with_config, diag_plugin_config_entry["config"])
    ...
    def test_run_diagnostics_plugin_exception(self):
        ...
        mock_plugin_instance.diagnose = MagicMock(side_effect=Exception("Diagnoser Died"))
        diagnoser.diagnostic_plugins["CrashingDiagnoser"] = mock_plugin_instance

        target_id = "service_beta"
        monitor_data_for_target = [{"target_id": target_id, "monitor_name": "SomeMonitor", "data": "value"}]
        all_monitor_data = {target_id: monitor_data_for_target}

        diag_plugin_config_entry = {"plugin": "CrashingDiagnoser", "config": {}}
        SUT_config.MONITORED_TARGETS = {
            target_id: {
                "enabled": True, "plugin": "SomeMonitor", "config": {},
                "diagnostic_plugins": [diag_plugin_config_entry],
                "recovery_plugins": []
            }
        }
        ...
        self.assertIn("Exception in plugin CrashingDiagnoser: Diagnoser Died", error_issue["details"])

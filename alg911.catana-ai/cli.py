...
def is_daemon_running():
    ...
    try:
        ...
        # A more robust check would involve `psutil` or `tasklist` and filtering by name.
            ...
            print(
                "Warning: Robust PID check on Windows requires 'psutil'. Assuming daemon might be running if PID file exists.",
                file=sys.stderr,
            )
...
def start_self_healing_daemon():
    ...
    if not SELF_HEALING_AVAILABLE:
        print(
            "Error: Self-healing module is not available. Cannot start daemon.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not self_healing_config.MODULE_ENABLED:
        print(
            "Info: Self-healing module is disabled in its configuration (self_healing_config.py). Daemon will not start.",
            file=sys.stdout,
        )
    ...
    if pid_file_exists:
        ...
        if running_pid:
            print(
                f"Self-Healing Daemon appears to be already running with PID: {running_pid}. (Checked via {SELF_HEAL_PID_FILE})"
            )
            print(
                "If this is incorrect, ensure the process is stopped and remove the PID file if necessary."
            )
            return
        else:
            print(
                f"Stale PID file found ({SELF_HEAL_PID_FILE}). Removing it before starting."
            )
            try:
                os.remove(SELF_HEAL_PID_FILE)
            except OSError as e:
                print(
                    f"Warning: Could not remove stale PID file {SELF_HEAL_PID_FILE}: {e}",
                    file=sys.stderr,
                )
    ...
    try:
        ...
        )  # Initialize it; it checks its own config.MODULE_ENABLED
        if not orchestrator.is_enabled:  # Double check if orchestrator disabled itself
            print(
                "Info: Self-Healing Orchestrator initialized but is internally disabled. Not starting healing loop.",
                file=sys.stdout,
            )
            sys.exit(0)
        ...
        try:
            ...
            print(
                f"Placeholder PID file created: {SELF_HEAL_PID_FILE} with PID {os.getpid()}"
            )
            print(
                "Warning: This is a placeholder. The self-healing process is NOT truly daemonized yet."
            )
        except IOError as e:
            print(
                f"Error: Could not write PID file {SELF_HEAL_PID_FILE}: {e}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def handle_self_heal_status(args):
    if not SELF_HEALING_AVAILABLE:
        print(
            "Error: Self-healing module is not available. Cannot get status.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if running_pid:
        print(
            f"  Daemon Process: Running (PID: {running_pid} from {SELF_HEAL_PID_FILE})"
        )
        if os.path.exists(status_file_path):
            try:
                ...
                print(
                    f"  Last Status Update: {datetime.datetime.fromtimestamp(status_data.get('last_updated_ts', 0)).strftime('%Y-%m-%d %H:%M:%S') if status_data.get('last_updated_ts') else 'N/A'}"
                )
                print(
                    f"  Daemon Reported Status: {status_data.get('daemon_status', 'N/A')}"
                )
                ...
                print(
                    f"    Total Active Issues: {status_data.get('active_issues_count', len(active_issues))}"
                )
                if active_issues:
                    for issue in active_issues[:3]:  # Show top 3
                        print(
                            f"    - Key: {issue.get('key')}, Attempts: {issue.get('attempts', 'N/A')}"
                        )
                else:
                ...
                if recovery_actions:
                    for action in recovery_actions[-3:]:  # Show last 3
                        ...
                        print(
                            f"    - [{ts}] Target: {action.get('target_id')}, Action: {action.get('action')}, Outcome: {action.get('outcome')}"
                        )
                else:
                ...
                print(
                    f"  Error: Could not parse status file: {status_file_path}. It might be corrupted."
                )
            except IOError as e:
                print(f"  Error: Could not read status file {status_file_path}: {e}")
        else:
            print(
                f"  Status file not found: {status_file_path}. Daemon might be starting or status not yet written."
            )
    else:
        if os.path.exists(SELF_HEAL_PID_FILE):
            print(
                f"  Daemon Process: Not Running (Stale PID file found: {SELF_HEAL_PID_FILE})"
            )
        else:
...
def main():
    ...
    sh_subparsers = sh_parser.add_subparsers(
        title="Self-Heal Actions", dest="sh_action", metavar="<action>"
    )

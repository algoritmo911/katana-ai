...
AGENT_LOG_PREFIX = shared_config.AGENT_LOG_PREFIX # Component prefix for logs from this agent
...
def load_json_file(file_path, default_value, log_prefix_override=None):
    ...
    if not os.path.exists(file_path):
        log_event(f"[{log_prefix_override or 'JSONLoad'}] File not found: {file_path}. Returning default.", "info", AGENT_LOG_PREFIX)
        return default_value
    try:
        with open(file_path, "r") as f:
            content = f.read()
            if not content.strip(): # File is empty
                log_event(f"[{log_prefix_override or 'JSONLoad'}] File is empty: {file_path}. Returning default.", "info", AGENT_LOG_PREFIX)
                return default_value
            data = json.loads(content)
        # log_event(f"[{log_prefix_override or 'JSONLoad'}] Loaded successfully from {file_path}.", "debug", AGENT_LOG_PREFIX)
        return data
    except json.JSONDecodeError:
        log_event(f"[{log_prefix_override or 'JSONLoad'}] Error decoding JSON from {file_path}. Returning default.", "error", AGENT_LOG_PREFIX)
        return default_value
    except Exception as e: # Catch any other read errors
        log_event(f"[{log_prefix_override or 'JSONLoad'}] Unexpected error loading {file_path}: {e}. Returning default.", "error", AGENT_LOG_PREFIX)
        return default_value
...
def save_json_file(file_path, data, log_prefix_override=None, indent=2):
    ...
    try:
        ...
        log_event(f"[{log_prefix_override or 'JSONSave'}] Successfully saved JSON to {file_path}.", "info", AGENT_LOG_PREFIX)
        return True
    except Exception as e:
        log_event(f"[{log_prefix_override or 'JSONSave'}] Error saving JSON to {file_path}: {e}", "error", AGENT_LOG_PREFIX)
        return False
...
def load_memory():
    ...
    if not isinstance(loaded_data, dict):
         log_event(f"Memory file {MEMORY_FILE} content was not a dictionary. Resetting memory state.", "warning", AGENT_LOG_PREFIX)
...
def load_commands_from_file(): return load_json_file(COMMANDS_FILE, [], "CommandsLoad")
def save_commands_to_file(commands_list): return save_json_file(COMMANDS_FILE, commands_list, "CommandsSave")
...
def initialize_katana_files():
    log_event("Initializing/Verifying Katana data files for MCP_v1...", "info", AGENT_LOG_PREFIX)
    ...
    for file_path, (default_content, expected_type, log_prefix_short) in files_to_initialize_or_verify.items():
        if not os.path.exists(file_path):
            ...
            if os.path.getsize(file_path) == 0:
                log_event(f"{file_path} is empty. Re-initializing.", "warning", AGENT_LOG_PREFIX)
                save_json_file(file_path, default_content, log_prefix_short)
            else:
                loaded_content = load_json_file(file_path, None, f"InitCheck{log_prefix_short[4:]}")
                if loaded_content is None or not isinstance(loaded_content, expected_type):
                    log_event(f"{file_path} is not a {expected_type.__name__} or is corrupted/unreadable. Re-initializing.", "warning", AGENT_LOG_PREFIX)
                    save_json_file(file_path, default_content, log_prefix_short)
                     ...
                     if not all(k in loaded_content for k in default_content.keys()):
                        log_event(f"{SYNC_STATUS_FILE} is missing essential keys. Re-initializing.", "warning", AGENT_LOG_PREFIX)
                        save_json_file(SYNC_STATUS_FILE, default_content, log_prefix_short)
                     else:
                        log_event(f"{file_path} exists and appears valid.", "debug", AGENT_LOG_PREFIX)
                else:
                    log_event(f"{file_path} exists and appears valid.", "debug", AGENT_LOG_PREFIX)

    global agent_memory_state
    agent_memory_state = load_memory() # Load memory after ensuring files are okay
    log_event("Katana data file initialization/verification complete.", "info", AGENT_LOG_PREFIX)
...
def process_pending_commands():
    ...
    if not isinstance(commands_on_disk, list):
        log_event(f"Command file {COMMANDS_FILE} does not contain a list. Skipping processing cycle.", "error", AGENT_LOG_PREFIX)
        return
    ...
    for command in commands_on_disk:
        ...
        if not command_id:
            log_event(f"Found command without ID: {command}. Skipping.", "warning", AGENT_LOG_PREFIX)
            continue

        if command_id not in processed_command_ids:
            new_commands_found = True
            log_event(f"Processing new command ID: {command_id}, Type: {command.get('command_details', {}).get('command_type', 'N/A')}", "info", AGENT_LOG_PREFIX)

            # Simulate processing: Update status and add to history
            command["status"] = "processed_by_agent"
            command["timestamp_processed_agent"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
    ...
    if new_commands_found:
        ...
        if save_commands_to_file(commands_on_disk): # Save back with updated statuses
             log_event(f"Updated command statuses in {COMMANDS_FILE}.", "info", AGENT_LOG_PREFIX)
        else:
             log_event(f"Failed to update command statuses in {COMMANDS_FILE}.", "error", AGENT_LOG_PREFIX)

        if history_modified:
            if save_history(current_history):
                log_event(f"Saved {len(current_history)} items to history file.", "info", AGENT_LOG_PREFIX)
            else:
                log_event(f"Failed to save history to {HISTORY_FILE}.", "error", AGENT_LOG_PREFIX)
...
def agent_main_loop(loop_interval_seconds=10):
    ...
    log_event(f"Katana Agent started. Will check for commands every {loop_interval_seconds} seconds.", "info", AGENT_LOG_PREFIX)
    try:
        while True:
        ...
        log_event("Katana Agent shutting down due to user interrupt.", "info", AGENT_LOG_PREFIX)
    except Exception as e:
        log_event(f"Katana Agent encountered a critical error: {e}", "critical", AGENT_LOG_PREFIX)
        # import traceback
...
if __name__ == '__main__':
    ...
    log_event("katana_agent.py: File initialization complete.", "info", AGENT_LOG_PREFIX)

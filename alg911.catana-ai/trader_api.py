...
def load_commands(file_path):
    ...
    try:
        ...
        log_event(f"Error decoding JSON from {file_path}. Returning empty list.", "error", COMPONENT_PREFIX)
        return []
    except Exception as e:
        log_event(f"Unexpected error loading {file_path}: {e}. Returning empty list.", "error", COMPONENT_PREFIX)
        return []
...
def post_trader_command():
    ...
    if not command_type:
        return jsonify({"status": "error", "message": "Missing 'command_type' in request"}), 400

    # Construct the command object
    new_command = {
        "command_id": str(uuid.uuid4()),
        "timestamp_received_api": datetime.datetime.now(datetime.timezone.utc).isoformat(),
    ...
    if save_commands(COMMANDS_FILE, commands_list):
        log_event(f"Received command: {command_type}. ID: {new_command['command_id']}", "info")
        return jsonify({
            "status": "success",
        ...
        return jsonify({"status": "error", "message": "Failed to save command to queue"}), 500
...
if __name__ == '__main__':
    ...
    if not os.path.isabs(COMMANDS_FILE): # If it's a relative path, make it absolute from script dir
        COMMANDS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), COMMANDS_FILE)

    log_event(f"Trader API starting. Commands will be written to: {COMMANDS_FILE}", "info")

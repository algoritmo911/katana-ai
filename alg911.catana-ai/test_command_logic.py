...
MEMORY_FILE = os.path.join(SCRIPT_DIR, "katana_memory.json") # For process_single_command to know where memory would be saved by agent
...
def find_unprocessed_command(commands_list):
    log_test_event(f"Searching for unprocessed command in list of {len(commands_list)} items.", "debug")
    ...
    for command_item in commands_list:
        if isinstance(command_item, dict) and not command_item.get("processed", False):
            log_test_event(f"Found unprocessed command: ID '{command_item.get('id')}'", "debug") # Corrected quotes
            return command_item
...
def process_single_command(command_dict, current_memory_data):
    ...
    log_test_event(f"Processing action: '{action}' with args: {args} for command ID: {command_id}", "info")
    ...
    if action == "remember":
        ...
        if key is not None:
            ...
            log_test_event(f"Remembered key '{key}' with value '{value}'. Memory updated.", "info")
            return {"status": "success", "result": f"Remembered key '{key}' with value '{value}'."}
        else:
            log_test_event("'remember' command missing 'key' in args.", "warning")
            return {"status": "failed", "result": "'remember' command requires a 'key' in args."}

    elif action == "ask":
        key = args.get("key")
        if key is not None:
            response = current_memory_data.get(key, "I don\'t remember that.")
            log_test_event(f"Asked about key '{key}'. Responded with: '{response}'.", "info")
            return {"status": "success", "result": response}
        else:
            log_test_event("'ask' command missing 'key' in args.", "warning")
            return {"status": "failed", "result": "'ask' command requires a 'key' in args."}

    else:
        log_test_event(f"Unknown command action: '{action}' for command ID: {command_id}", "warning")
...
def update_command_in_list(commands_list, command_id, updates_dict):
    log_test_event(f"Attempting to update command ID '{command_id}' in list with updates: {updates_dict}", "debug")
    if not isinstance(commands_list, list):
...
def add_to_history_list(history_list, command_id, command_action, command_args, executed_at, status, result_message):
    # This function just appends to an in-memory list for testing purposes.
    # The agent will handle loading/saving history.json.
    log_test_event(f"Adding command {command_id} to in-memory history list with status: {status}", "debug")
    ...
    log_test_event(f"Command {command_id} added to in-memory history list. New length: {len(history_list)}", "debug")
    return True
...
if __name__ == "__main__":
    ...
    # Test Data
    sample_commands = [
        {"id": "cmd_001", "command": "remember", "args": {"key": "greeting", "value": "Hello Katana!"}, "created_at": "2024-01-01T10:00:00Z", "processed": False},
        {"id": "cmd_002", "command": "ask", "args": {"key": "greeting"}, "created_at": "2024-01-01T10:01:00Z", "processed": False},
        {"id": "cmd_003", "command": "ask", "args": {"key": "non_existent_key"}, "created_at": "2024-01-01T10:02:00Z", "processed": False},
        {"id": "cmd_004", "command": "unknown_action", "args": {}, "created_at": "2024-01-01T10:03:00Z", "processed": False},
        {"id": "cmd_005", "command": "remember", "args": {"key": "farewell"}, "created_at": "2024-01-01T10:04:00Z", "processed": True} # Already processed
    ]
    current_memory = {}
    current_history = []
    ...
    assert cmd_to_run and cmd_to_run["id"] == "cmd_001", "Test 1 FAILED: Did not find cmd_001"
    print("Test 1 PASSED.")
    ...
    if cmd_to_run:
        ...
        assert exec_result["status"] == "success", "Test 2 FAILED: 'remember' status not success"
        assert current_memory.get("greeting") == "Hello Katana!", "Test 2 FAILED: Memory not updated correctly"
        update_command_in_list(sample_commands, cmd_to_run["id"], {"processed": True, "executed_at": "NOW", "status_after_execution": exec_result["status"]})
        add_to_history_list(current_history, cmd_to_run["id"], cmd_to_run["command"], cmd_to_run["args"], "NOW", exec_result["status"], exec_result["result"])
    print("Test 2 PASSED.")
    ...
    cmd_to_run = find_unprocessed_command(sample_commands)
    print(f"Found command to run: {cmd_to_run.get('id') if cmd_to_run else 'None'}")
    assert cmd_to_run and cmd_to_run["id"] == "cmd_002", "Test 3 FAILED: Did not find cmd_002"
    print("Test 3 PASSED.")
    ...
    if cmd_to_run:
        exec_result = process_single_command(cmd_to_run, current_memory)
        print(f"Execution result: {exec_result}")
        assert exec_result["status"] == "success" and exec_result["result"] == "Hello Katana!", "Test 4 FAILED: 'ask' did not return correct value"
        update_command_in_list(sample_commands, cmd_to_run["id"], {"processed": True, "executed_at": "NOW", "status_after_execution": exec_result["status"]})
        add_to_history_list(current_history, cmd_to_run["id"], cmd_to_run["command"], cmd_to_run["args"], "NOW", exec_result["status"], exec_result["result"])
    print("Test 4 PASSED.")
    ...
    if cmd_to_run:
        exec_result = process_single_command(cmd_to_run, current_memory)
        print(f"Execution result: {exec_result}")
        assert exec_result["status"] == "success" and exec_result["result"] == "I don\'t remember that.", "Test 5 FAILED: 'ask' for non-existent key failed"
        update_command_in_list(sample_commands, cmd_to_run["id"], {"processed": True, "executed_at": "NOW", "status_after_execution": exec_result["status"]})
        add_to_history_list(current_history, cmd_to_run["id"], cmd_to_run["command"], cmd_to_run["args"], "NOW", exec_result["status"], exec_result["result"])
    print("Test 5 PASSED.")
    ...
    if cmd_to_run:
        exec_result = process_single_command(cmd_to_run, current_memory)
        print(f"Execution result: {exec_result}")
        assert exec_result["status"] == "failed", "Test 6 FAILED: Unknown command did not fail"
        update_command_in_list(sample_commands, cmd_to_run["id"], {"processed": True, "executed_at": "NOW", "status_after_execution": exec_result["status"]})
        add_to_history_list(current_history, cmd_to_run["id"], cmd_to_run["command"], cmd_to_run["args"], "NOW", exec_result["status"], exec_result["result"])
    print("Test 6 PASSED.")
    ...
    cmd_to_run = find_unprocessed_command(sample_commands)
    print(f"Found command to run: {cmd_to_run.get('id') if cmd_to_run else 'None'}")
    assert cmd_to_run is None, "Test 7 FAILED: Found an unprocessed command when none should be left"
    print("Test 7 PASSED.")

...
class Planner:
    ...
    def _create_plan_for_acquire_and_structure_knowledge(self, goal: Dict[str, Any], plan_id: str) -> Plan:
        """Creates the plan for the self-generated goal of acquiring knowledge."""
        details = goal.get('details', {})
        topic = details.get('topic', 'general_knowledge')
        return Plan(
            ...
            steps=[
                Command(
                    ...
                    type=CommandType.N8N_WEBHOOK,
                    params={
                        "webhook_url": os.environ.get("N8N_WEBHOOK_SEARCH_KNOWLEDGE", "https://n8n.katana.foo/webhook/search-knowledge"),
                        "payload": {"topic": topic}
                    }
                ),
                Command(
                    name="neurovault.add_knowledge",
                    type=CommandType.LOCAL_PYTHON,
                    # This command expects the output of the n8n webhook (a list of concepts)
                    # The executor will need to handle passing this data.
                    params={}
                )
            ]
        )

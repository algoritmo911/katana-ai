...
def _validate_json_file(filepath, diagnostic_log):
    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            logging.info(f"File JSON is valid: {filepath}")
...
def check_katana_files(diagnostic_log):
    ...
    logging.info("Starting validation of Katana files.")
...
def check_rclone(diagnostic_log):
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            logging.info(f"rclone is accessible. Remotes: {remotes}")
        ...
        logging.error("rclone command not found. Ensure rclone is installed and in PATH.")
        rclone_status["error"] = "rclone command not found"
        log_entry["status"] = "not_found_error"
        rclone_status["installed"] = False
    except subprocess.TimeoutExpired:
        logging.error("rclone command timed out.")
        ...
        logging.error(f"An unexpected error occurred while checking rclone: {e}")
    ...
    logging.info("rclone check completed.")
...
def run_healthcheck():
    ...
    logging.info("Health check process started.")
    ...
    try:
        ...
        logging.info(f"Successfully generated {DIAGNOSTIC_LOG_FILE}")
    except IOError as e:
        logging.error(f"Failed to write diagnostic log: {e}")

    logging.info("Health check process completed.")
